...
// -- GET /sequence -- read a dataset's current counter value, no mint -----------
//
// Query params: graph?, counterIri? -- same convention defaults as POST /sequence/mint.
// Response: { dataset, value: number|null, graph, counterIri }
//   value is null if the counter has never been minted from yet.

app.get('/sequence', async (req, res) => {
  const dataset    = req.datasetOverride || DATASET
  const graphIri   = (req.query?.graph && String(req.query.graph).trim())
    ? String(req.query.graph).trim() : `urn:${dataset}:holons`
  const counterIri = (req.query?.counterIri && String(req.query.counterIri).trim())
    ? String(req.query.counterIri).trim() : `urn:${dataset}:sequence-counter`

  try {
    const value = await readSequenceValue({ sparqlEndpoint: req.sparqlEndpoint, graphIri, counterIri })
    return res.json({ dataset, value, graph: graphIri, counterIri })
  } catch (err) {
    console.error('[Bridge] /sequence error:', err)
    return res.status(500).json({ error: 'Sequence read failed', message: err.message })
  }
})
...
//
// Extracted 2026-07-19 into lib/routes/holon-lifecycle.js -- first slice of
// the router split (Kurt, 2026-07-19: router-splitting now, deeper state
// refactor deferred until the Wednesday founders session settles Choice A/B
// and the write path stops moving). Structural only -- every route body is
// unchanged; getter functions give the extracted router live access to this
// file's still-mutable dataset config without requiring any other route in
// this file to change in the same pass.
//
// ROADMAP for the remaining extractions (not yet done, same pattern applies):
//   lib/routes/sparql.js         -- /query, /update, /sparql-select,
//                                    /sparql-construct, /sparql-update,
//                                    /describe, /graphs, /graph, /validate
//   lib/routes/dataset-admin.js  -- /dataset, /fuseki-url, /shacl-mode,
//                                    /dataset-holon-iri, /reload,
//                                    /sequence/mint, /sequence, /datasets
//   lib/routes/named-registry.js -- /named-query, /named-queries,
//                                    /named-rule, /named-rules, /rule,
//                                    /graph-op
//   lib/routes/pipeline.js       -- /pipeline, /pipelines, /ingest,
//                                    /pipeline-run, /message/:id
//                                    ([NON-CANONICAL] -- pending WG IV)
//   lib/routes/scheduler.js      -- GET /scheduler, POST /scheduler/reload
//   lib/routes/meta.js           -- /description, /health, /registry,
//                                    /registry/refresh
// Each follows the same recipe: extract the block, app.X( -> router.X(,
// replace bare DATASET/JENA_*/etc. reads with getter calls passed in at
// mount time, add the router.use(...) call here in the same position the
// routes used to occupy (preserves middleware ordering exactly).

app.use(createHolonLifecycleRouter({
  getDataset:         () => DATASET,
  getJenaBase:         () => JENA_BASE,
  getJenaSparql:       () => JENA_SPARQL,
  getJenaGsp:          () => JENA_GSP,
  getDatasetHolonIri:  () => DATASET_HOLON_IRI
}))
...
app.use((_req, res) => {
  res.status(404).json({
    ...
    available: [
      ...
      'POST /sequence/mint', 'GET  /sequence',
