 ...
 *
 * Implements the eleven verbs whose contract is defined in
 ...
 *     authorise() walks holon:parentHolon UPWARD from the target holon and
...
import { runQuery, runConstruct, pushToGraph } from './sparql.js'
 ...
 * holon:parentHolon upward and stopping at the first RoleBinding found.
...
export async function resolveCapabilities(sparqlEndpoint, holonIri, actorIri) {
  ...
  while (current && !visited.has(current)) {
    ...
    const parentQuery = `
      PREFIX holon: <${HOLON_NS}>
      SELECT ?parent WHERE {
        GRAPH ?registryGraph { <${current}> holon:parentHolon ?parent }
      }`
 ...
 * @param {{baseIri: string, label: string, actor: {iri: string}}} params
 * @returns {Promise<string>} serialized HolonDataBook
 */
export async function createRootHolon(conn, { baseIri, label, actor }) {
...
<${childIri}> a holon:Holon ;
    holon:parentHolon <${parentHolonIri}> ;
...
`.trim()
  await pushToGraph(conn.gspEndpoint, graphsFor(parentHolonIri).scene, resolveTurtle, 'append')
  // NOTE: production implementation must also DELETE the resolved
  // holon:portalPotential triple via sparql_update (DELETE/INSERT WHERE) --
  // omitted here pending a sparql_update wrapper in sparql.js.
...
export async function editMetadata(conn, holonIri, patch, actor) {
  ...
  const triples = []
  if (patch.title) triples.push(`<${holonIri}> <http://www.w3.org/2000/01/rdf-schema#label> "${patch.title.replace(/"/g, '\\"')}" .`)
  if (patch.description) triples.push(`<${holonIri}> <http://www.w3.org/2000/01/rdf-schema#comment> "${patch.description.replace(/"/g, '\\"')}" .`)
  if (patch.status) triples.push(`<${holonIri}> <${HOLON_NS}status> <${HOLON_NS}${patch.status}Status> .`)

  const turtle = `@prefix holon: <${HOLON_NS}> .\n${triples.join('\n')}`
  // NOTE: production version must DELETE the prior title/description/status
  // triples via sparql_update before INSERTing -- pushToGraph 'append' alone
  // would leave stale values alongside new ones. Flagged as a TODO pending
  // a sparql_update wrapper in sparql.js.
  await pushToGraph(conn.gspEndpoint, null, turtle, 'append')
...
`.trim()
  await pushToGraph(conn.gspEndpoint, null, turtle, 'append')
  // NOTE: also needs a DELETE of the prior status triple + removal of the
  // portal reference from the parent's scene graph -- sparql_update TODO,
  // same as editMetadata above.
