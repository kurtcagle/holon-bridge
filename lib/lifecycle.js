 ...
 *   - Reparenting: moveHolon() changes holon:parentHolon directly. Any
 *     holon may be flagged holon:rootLocked true (typically set at
 *     createRootHolon time) to mark it as an intentional root structure --
 *     a registry, a corridor spine, a journeys index -- that should never
 *     be silently reparented by a stray moveHolon call. A locked holon can
 *     still be moved with { force: true }, which is a deliberate,
 *     visible override rather than a separate bypass path.
 */
...
export async function deleteHolon(conn, holonIri, opts) {
  ...
  const parentQuery = `
    PREFIX holon: <${HOLON_NS}>
    SELECT ?parent WHERE { <${holonIri}> holon:parentHolon ?parent }`
...
/**
 * Reparents a holon: changes its holon:parentHolon from whatever it
...
export async function moveHolon(conn, holonIri, opts) {
  ...
  const stateQuery = `
    ...
    SELECT ?parent ?rootLocked WHERE {
      OPTIONAL { <${holonIri}> holon:parentHolon ?parent }
  ...
  await replaceTriples(
    conn.sparqlEndpoint, null,
    `<${holonIri}> holon:parentHolon ?old .`,
    `<${holonIri}> holon:parentHolon <${newParentIri}> .`,
    { prefixes: PREFIXES, wherePattern: `OPTIONAL { <${holonIri}> holon:parentHolon ?old } .` }
  )
