 ...
 *   - navigateAgent (verb 18, added 2026-07-10) closes the same ad-hoc-
 *     SPARQL-UPDATE gap proposeAgentPropertyUpdate closed for numeric
 *     properties, but for holon:currentLocation: every prior change to
 *     that property (including the ones formGroup/joinGroup/leaveGroup
 *     make internally) recorded where an agent currently is, never when
 *     they moved or what path they took. Writes a holon:NavigateEvent
 *     (holon:previousLocation?, holon:newLocation, holon:occurredAt) per
 *     move -- trajectory is then just an ORDER BY holon:occurredAt over
 *     that agent's NavigateEvents, no explicit chain needed. Self-service
 *     when actor.iri === agentIri, same split as joinGroup/leaveGroup.
 */
...
/**
 * Move an agent's holon:currentLocation to a new holon, writing a
 * holon:NavigateEvent that records where they came from and where they
 * went. Closes the same gap proposeAgentPropertyUpdate closed for numeric
 * properties (see that verb's header comment) for the one property that
 * was still being changed via ad hoc SPARQL UPDATE with no gate and no
 * event trail: every currentLocation change prior to this verb -- including
 * everything formGroup/joinGroup/leaveGroup do internally -- left a record
 * of where an agent currently is, never of when they moved or what path
 * they took to get there.
 *
 * Trajectory is queryable after the fact with no additional bookkeeping:
 *
 *   SELECT ?loc ?at WHERE { GRAPH <eventsGraph> {
 *     ?e a holon:NavigateEvent ; holon:targetHolon <agentIri> ;
 *        holon:newLocation ?loc ; holon:occurredAt ?at
 *   } } ORDER BY ?at
 *
 * Chronological ordering falls out of holon:occurredAt directly -- no
 * explicit next/previous chain between events is written or needed, unlike
 * holon:nextHolon (an authored reading order over CONTENT, asserted once,
 * true for any agent) or holon:evolvesTo (version succession of a single
 * holon). A trajectory is agent-specific and incidental, not authored
 * structure, so it doesn't belong on either of those predicates -- it gets
 * its own event class instead, following the same
 * validate/authorise/execute/assert pattern as every flat-graph verb here.
 *
 * Does not require newLocationIri to already exist as a fully-registered
 * holon in any strict sense -- matches moveHolon()'s permissiveness toward
 * newParentIri (this dataset's holons are asserted piecemeal across many
 * pushes, not through a single registration gate, so requiring
 * pre-existence here would reject perfectly valid moves to a holon minted
 * moments earlier in the same session). Does require agentIri to already
 * be a registered holon:Agent, mirroring joinGroup's "must be a registered
 * holon:Group" check on the membership side of the same currentLocation
 * property. Also refuses a no-op move (newLocationIri === current
 * location) -- an agent "navigating" to where they already are is not an
 * event worth logging.
 *
 * Authorisation: self-service -- no capability check when actor.iri ===
 * agentIri (an agent moving itself is the common case). Write on
 * datasetAnchor(conn) is required when the actor is moving someone else
 * (e.g. a GM/narrator directing an NPC), matching the exact self-service
 * split already used by joinGroup/leaveGroup.
 *
 * Operates on the flat urn:{dataset}:holons / urn:{dataset}:events graphs
 * (see proposeAgentPropertyUpdate and this file's header) -- pass
 ...
 * @param {string} agentIri
 * @param {{newLocationIri: string, actor: {iri: string}}} opts
 * @returns {Promise<string>} serialized HolonDataBook
 */
export async function navigateAgent(conn, agentIri, opts) {
  const { newLocationIri, actor } = opts
  const holonsGraph = conn.holonsGraph ?? `urn:${conn.dataset}:holons`
  const eventsGraph = conn.eventsGraph ?? `urn:${conn.dataset}:events`
  ...
  const agentQuery = `
    ...
    SELECT ?loc WHERE {
      GRAPH <${holonsGraph}> {
        <${agentIri}> a holon:Agent .
        OPTIONAL { <${agentIri}> holon:currentLocation ?loc }
      }
    } LIMIT 1`
  const { bindings } = await runQuery(conn.sparqlEndpoint, agentQuery)
  if (bindings.length === 0) {
    throw new CommandRejected(`<${agentIri}> is not a registered holon:Agent`, 'NavigateAgentCommand')
  }
  const previousLocation = bindings[0]?.loc?.value ?? null

  if (newLocationIri === previousLocation) {
    throw new CommandRejected(`<${agentIri}> is already at <${newLocationIri}>`, 'NavigateAgentCommand')
  }

  await runUpdate(conn.sparqlEndpoint, `
    DELETE { GRAPH <${holonsGraph}> { <${agentIri}> <${HOLON_NS}currentLocation> ?old } }
    INSERT { GRAPH <${holonsGraph}> { <${agentIri}> <${HOLON_NS}currentLocation> <${newLocationIri}> } }
    WHERE  { OPTIONAL { GRAPH <${holonsGraph}> { <${agentIri}> <${HOLON_NS}currentLocation> ?old } } }
  `.trim())
...
<${eventIri}> a holon:NavigateEvent ;
    rdfs:label "Navigated${previousLocation ? ` from <${previousLocation}>` : ''} to <${newLocationIri}>" ;
    holon:agent <${actor.iri}> ;
    holon:targetHolon <${agentIri}> ;
    ${previousLocation ? `holon:previousLocation <${previousLocation}> ;\n    ` : ''}holon:newLocation <${newLocationIri}> ;
    holon:occurredAt "${now}"^^xsd:dateTime ;
    holon:wasSuccessful true .
`.trim()
  await pushToGraph(conn.gspEndpoint, eventsGraph, eventTurtle, 'append')
  ...
  return buildDataBook(
    stampProcess({ id: `${agentIri}#navigate`, title: 'Agent navigated', type: 'holon-agent-navigation' }, actor),
    [{ id: 'navigate', label: `<${agentIri}> -> <${newLocationIri}>`, content: eventTurtle }]
  )
}
