 ...
 *
 * Implements the eighteen verbs whose contract is defined in
 ...
 *     they hold. createRootHolon() self-grants actor an Owner binding on
 *     the new root at creation time (fix for #8): a fresh root has no
 *     ancestor to inherit a binding from the way promoteEntity's realised
 *     child does, so without this nobody -- including the actor who just
 *     created it -- ever holds any capability on it, and designateAgent's
 *     own Grant check could never succeed to establish one after the fact.
 *   - Registry graph (fixed 2026-09-14, confirmed via live MCP testing of
 *     the #8 fix): createRootHolon()'s and designateAgent()'s RoleBinding
 *     writes were pushed with graphIri: null -- Fuseki's default graph --
 *     while resolveCapabilities() looks them up wrapped in
 *     `GRAPH ?registryGraph { ... }`. SPARQL's GRAPH ?var pattern only
 *     ever matches NAMED graphs, never the default graph, so those
 *     bindings were written successfully but permanently invisible to
 *     every capability check -- the #8 fix's write path was correct, the
 *     read path it depends on could never see it. Confirmed live: a
 *     freshly-granted Owner binding was queryable with a plain triple
 *     pattern but returned zero rows wrapped in GRAPH ?g. Both verbs now
 *     push to a real named graph via the new registryGraphIri(conn)
 *     helper (urn:{dataset}:registry); resolveCapabilities() needed no
 *     change, since GRAPH ?registryGraph already matches any named graph
 *     containing the pattern. Extended same day to close the rest of the
 *     same hole: promoteEntity()'s child registration, moveHolon()'s
 *     reparenting write, addProjection()'s projection triple, and
 *     editMetadata()'s/deleteHolon()'s status-and-metadata writes all now
 *     push/replaceTriples into registryGraphIri(conn) too, rather than
 *     graphIri: null. Every READ that depended on those same triples living
 *     in the (bare, unnamed) default graph -- moveHolon()'s own stateQuery,
 *     deleteHolon()'s parentQuery, purgeHolon()'s statusQuery -- was
 *     updated in lockstep to wrap in GRAPH ?g, matching the convention
 *     resolveCapabilities() and lib/holon.js's fetchHolonProjection()
 *     (GET /holon) already used. Net effect: every triple this file writes
 *     outside a holon's own schema/scene/events graphs now lives in one
 *     named registry graph, and every read that needs to find it searches
 *     named graphs rather than assuming the default one -- the invisibility
 *     class of bug is closed everywhere in this file, not just for
 *     RoleBindings. Known limitation: any holon created before this fix
 *     landed still has its registration data in the old default graph;
 *     editMetadata/deleteHolon calls against those pre-fix holons will
 *     insert a new triple into the registry graph without finding-and-
 *     removing the old default-graph one, leaving an orphaned duplicate.
 *     No migration step exists for this yet.
 ...
 *     record of its starting values. Same validate-before-write
 ...
 *   - Authorisation on the flat-graph verbs (proposeAgentPropertyUpdate,
 *     createAgent, formGroup, joinGroup, leaveGroup -- added 2026-07-10,
 *     closing a gap flagged the same day these verbs' schema entries were
 *     written): these verbs have no holonIri to walk the containment tree
 *     from, since agents and groups in a flat Adventure-Mode dataset are
 *     not holons in the containment hierarchy at all. Each now gates
 *     against a single per-dataset anchor -- conn.datasetHolonIri if given,
 *     else the convention urn:{dataset}:root -- via the existing
 *     authorise()/resolveCapabilities() machinery, so a RoleBinding granted
 *     once at that anchor covers every flat-graph operation in the dataset.
 *     Group verbs stay low-friction for the common case: an actor acting on
 *     their OWN membership (forming a party that includes themselves,
 *     joining themselves, leaving themselves) needs no capability at all --
 *     only operations that move or remove members OTHER than the actor
 *     require Write on the dataset anchor. proposeAgentPropertyUpdate and
 *     createAgent always require Write; they change game-state numbers or
 *     mint new agents, which isn't something an actor should self-authorise
 *     even for their own agent.
 *   - navigateAgent() (verb 18, added 2026-07-11) generalises the
 *     holon:VisitEvent / holon:nextVisit chain pattern already present by
 *     hand in this dataset's event history (the Kim Meades Bonn -> Germany
 *     chain, written via raw SPARQL UPDATE before this verb existed) into
 *     an actual gated operation -- closing the same kind of provenance gap
 *     proposeAgentPropertyUpdate closed for numeric properties, this time
 *     for holon:currentLocation changes. Destination existence is checked
 *     before any write; the new VisitEvent is linked from whatever
 *     VisitEvent currently has no holon:nextVisit for that agent (the
 *     chain tail), or starts a fresh chain if none exists yet.
 *     Authorisation follows the same self-service posture as
 *     joinGroup/leaveGroup: no capability check when an agent moves
 *     itself, Write on the dataset anchor otherwise.
 */
...
/**
 * Build (but do not push) the Turtle for a single holon:RoleBinding triple.
 * Shared by designateAgent() (delegated grants, checked against the
 * grantor's own capability set) and createRootHolon() (the self-granted
 * Owner binding that bootstraps a fresh root -- see the "ACL" bullet in
 * this file's header and issue #8). Pushing is left to the caller, since
 * each writes this alongside a different sibling triple in the same
 * request (an agent-registration triple for designateAgent, the root
 * registration triple for createRootHolon).
 *
 * @param {string} holonIri
 * @param {string} agentIri
 * @param {string} grantedByIri
 * @param {string[]} capabilities
 * @returns {{bindingIri: string, turtle: string}}
 */
function roleBindingTurtle(holonIri, agentIri, grantedByIri, capabilities) {
  const bindingIri = `urn:role-binding:${crypto.randomUUID()}`
  const turtle = `
@prefix holon: <${HOLON_NS}> .

<${bindingIri}> a holon:RoleBinding ;
    holon:boundAgent <${agentIri}> ;
    holon:boundHolon <${holonIri}> ;
    ${capabilities.map(c => `holon:capability holon:${c}`).join(' ;\n    ')} ;
    holon:grantedBy <${grantedByIri}> .
`.trim()
  return { bindingIri, turtle }
}
...
/**
 * Resolve the anchor holon that gates the flat-graph verbs
 * (proposeAgentPropertyUpdate, createAgent, formGroup, joinGroup,
 * leaveGroup). These verbs operate on urn:{dataset}:holons/:events, not a
 * per-holon containment subtree, so there is no natural holonIri to walk
 * authorise() from -- a single per-dataset anchor stands in for it instead.
 * conn.datasetHolonIri lets a deployment point at a real registered holon
 * (e.g. the dataset's createRootHolon-minted root) to reuse an existing
 * RoleBinding; otherwise the convention urn:{dataset}:root is used, and a
 * RoleBinding must be designated there (via designateAgent) for any
 * capability check against it to succeed.
 *
 * @param {{dataset: string, datasetHolonIri?: string}} conn
 * @returns {string}
 */
function datasetAnchor(conn) {
  return conn.datasetHolonIri ?? `urn:${conn.dataset}:root`
}

/**
 * Resolve the IRI of the "registry" graph -- where holon registration
 * records and RoleBindings live outside a holon's own schema/scene/events
 * triad. Fix for #8's follow-up bug (2026-09-14): resolveCapabilities()
 * wraps its RoleBinding and containment lookups in GRAPH ?registryGraph,
 * which -- per SPARQL semantics -- only ever matches NAMED graphs, never
 * Fuseki's default graph. createRootHolon()/designateAgent() previously
 * pushed with graphIri: null (the default graph), so a freshly-written
 * binding was real but permanently invisible to every capability check.
 * conn.registryGraph lets a deployment override the convention explicitly
 * (mirrors conn.datasetHolonIri on datasetAnchor() above); otherwise
 * urn:{dataset}:registry is used.
 *
 * @param {{dataset: string, registryGraph?: string}} conn
 * @returns {string}
 */
function registryGraphIri(conn) {
  return conn.registryGraph ?? `urn:${conn.dataset}:registry`
}
...
/**
 * @param {{sparqlEndpoint: string, gspEndpoint: string, dataset?: string, registryGraph?: string}} conn
...
`.trim()
  ...
  // Fix for #8: a fresh root has no ancestor to inherit a RoleBinding from
  // (unlike promoteEntity's realised child, which inherits the parent's
  // Owner binding via containment) -- without an explicit binding here,
  // nobody, including actor, would ever hold any capability on this holon,
  // and designateAgent's own Grant check could never succeed to establish
  // one after the fact. actor is self-granted Owner, grantedBy themselves:
  // there is no other grantor at the moment a root is first created, and
  // Owner already implies Grant/Promote/Write/Read via CAPABILITY_IMPLIES.
  // Written in the same request as the root registration triple so a root
  // is never observably registered without also being owned.
  const { turtle: bindingTurtle } = roleBindingTurtle(baseIri, actor.iri, actor.iri, ['Owner'])

  // Registry-graph fix (2026-09-14, follow-up to #8, see registryGraphIri()
  // above): must be a real named graph, not null/default, or
  // resolveCapabilities()'s GRAPH ?registryGraph lookup can never see it.
  await pushToGraph(conn.gspEndpoint, registryGraphIri(conn), `${turtle}\n\n${bindingTurtle}`, 'append')
  ...
  return buildDataBook(
    ...
    [
      ...
      { id: 'owner-binding', label: 'Bootstrap Owner RoleBinding (fix for #8)', content: bindingTurtle }
    ]
  )
}
...
`.trim()
  // Registry-graph fix (2026-09-14, same class of bug as #8's follow-up):
  // the child's isPartOf link must land in a NAMED graph, or
  // resolveCapabilities()'s ancestor-walk parentQuery (GRAPH ?registryGraph)
  // can never find it -- meaning a promoted child would never actually
  // inherit its parent's Owner binding despite the design intent documented
  // in this file's header.
  await pushToGraph(conn.gspEndpoint, registryGraphIri(conn), childTurtle, 'append')
...
/**
 * @param {{gspEndpoint: string, dataset?: string, registryGraph?: string}} conn
 * @param {string} holonIri
...
`.trim()
  // Registry-graph fix (2026-09-14): lib/holon.js's fetchHolonProjection()
  // (GET /holon) reads a holon's own triples via GRAPH ?g -- a named-graph
  // pattern, same convention as resolveCapabilities() -- so this must not
  // land in the default graph either.
  await pushToGraph(conn.gspEndpoint, registryGraphIri(conn), turtle, 'append')
...
export async function editMetadata(conn, holonIri, patch, actor) {
  ...
  const triples = []
  // Registry-graph fix (2026-09-14): createRootHolon/promoteEntity now
  // write a holon's label/status into the registry graph, not the default
  // graph -- these three replaceTriples calls must target the same graph
  // or they'd silently insert a duplicate into the wrong graph instead of
  // replacing the real value.
  const registryGraph = registryGraphIri(conn)
  ...
  if (patch.title) {
    ...
    await replaceTriples(
      conn.sparqlEndpoint, registryGraph,
...
export async function deleteHolon(conn, holonIri, opts) {
  ...
  const PREFIXES = `PREFIX holon: <${HOLON_NS}>`
  // Registry-graph fix (2026-09-14): status now lives in the registry
  // graph (see createRootHolon/registryGraphIri), not the default graph.
  await replaceTriples(
    conn.sparqlEndpoint, registryGraphIri(conn),
    `<${holonIri}> holon:status ?old .`,
  ...
  const parentQuery = `
    SELECT ?parent WHERE {
      GRAPH ?g { <${holonIri}> ?viaProp ?parent . FILTER(?viaProp IN (${containmentList})) }
    } LIMIT 1`
...
export async function purgeHolon(conn, holonIri, opts) {
  ...
  const statusQuery = `
    PREFIX holon: <${HOLON_NS}>
    SELECT ?status WHERE { GRAPH ?g { <${holonIri}> holon:status ?status } }`
...
`.trim()
  const { bindingIri, turtle: bindingTurtle } = roleBindingTurtle(holonIri, agent.iri, grantedBy.iri, agent.capability)
  const turtle = `${agentTurtle}\n\n${bindingTurtle}`
...
export async function moveHolon(conn, holonIri, opts) {
  ...
  const stateQuery = `
    ...
    SELECT ?parent ?viaProp ?rootLocked WHERE {
      OPTIONAL { GRAPH ?g1 { <${holonIri}> ?viaProp ?parent . FILTER(?viaProp IN (${containmentList})) } }
      OPTIONAL { GRAPH ?g2 { <${holonIri}> holon:rootLocked ?rootLocked } }
    } LIMIT 1`
  ...
  const PREFIXES  = `PREFIX holon: <${HOLON_NS}>`
  // Registry-graph fix (2026-09-14, follow-up to #8): must land in a named
  // graph, or resolveCapabilities()'s ancestor-walk parentQuery can never
  // find the new link, and neither can this same stateQuery above on a
  // future call.
  await replaceTriples(
    conn.sparqlEndpoint, registryGraphIri(conn),
 ...
 *
 * Authorisation (added 2026-07-10): gates on datasetAnchor(conn) --
 * conn.datasetHolonIri, or the urn:{dataset}:root convention -- rather
 * than on agentIri itself, since agentIri is not a holon in the
 * containment tree. Always requires Write, even when actor.iri ===
 * agentIri: unlike group membership, a numeric game-state change (health,
 * wealth) is not something an actor should be able to self-authorise just
 * because it targets their own agent.
 *
 ...
 *
 * @param {{sparqlEndpoint: string, gspEndpoint: string, jenaBase: string, dataset: string, datasetHolonIri?: string, holonsGraph?: string, eventsGraph?: string, shapesGraph?: string}} conn
 * @param {string} agentIri
...
export async function proposeAgentPropertyUpdate(conn, agentIri, opts) {
  ...
  await authorise(conn.sparqlEndpoint, datasetAnchor(conn), actor, 'Write')
 ...
 *
 * Authorisation (added 2026-07-10): gates on datasetAnchor(conn), always
 * requiring Write -- minting a brand-new agent is never something an actor
 * self-authorises, even for a persona meant to represent themselves.
 *
 ...
 *
 * @param {{sparqlEndpoint: string, gspEndpoint: string, jenaBase: string, dataset: string, datasetHolonIri?: string, holonsGraph?: string, eventsGraph?: string, ontologyGraph?: string, shapesGraph?: string}} conn
 * @param {{
 *   agentIri: string,
 *   label: string,
 *   agentKind: string,
 ...
 *
 * Authorisation (added 2026-07-10): self-service by design, since forming a
 * party is meant to be a common, low-friction operation. No capability
 * check at all if every entry in memberIris is the actor themselves --
 * i.e. actor.iri is the ONLY member (a solo group is really only useful as
 * a precursor to others joining, but costs nothing to allow freely). As
 * soon as memberIris includes anyone other than the actor, Write on
 * datasetAnchor(conn) is required -- an actor can freely invite themselves
 * into existence as a group of one, but cannot unilaterally conscript other
 * agents into a party without the dataset-level capability.
 *
 * @param {{sparqlEndpoint, gspEndpoint, jenaBase, dataset, datasetHolonIri?, holonsGraph?, eventsGraph?}} conn
...
export async function formGroup(conn, opts) {
  ...
  const othersInvolved = memberIris.some(m => m !== actor.iri)
  if (othersInvolved) {
    await authorise(conn.sparqlEndpoint, datasetAnchor(conn), actor, 'Write')
  }
 ...
 *
 * Authorisation (added 2026-07-10): self-service -- no capability check
 * when actor.iri === memberIri (an agent joining themselves to a group is
 * the common case and should stay frictionless). Write on
 * datasetAnchor(conn) is required when the actor is adding someone else.
 *
 * @param {{sparqlEndpoint, gspEndpoint, dataset, datasetHolonIri?, holonsGraph?, eventsGraph?}} conn
 ...
 *
 * Authorisation (added 2026-07-10): self-service -- no capability check
 * when actor.iri === memberIri (an agent removing themselves from a group
 * is the common case, e.g. splitting off from a party, and should stay
 * frictionless). Write on datasetAnchor(conn) is required when the actor
 * is removing someone else -- e.g. a GM ejecting a member, or force-
 * dissolving a party from outside it.
 *
 * @param {{sparqlEndpoint, gspEndpoint, dataset, datasetHolonIri?, holonsGraph?, eventsGraph?}} conn
 * @param {string} groupIri
 * @param {string} memberIri
...
/**
 * Move an agent to a new holon, writing a holon:VisitEvent that chains off
 * whatever VisitEvent currently represents that agent's most recent visit
 * (linked via holon:nextVisit), then updates the agent's own
 * holon:currentLocation to match. Closes the same provenance gap
 * proposeAgentPropertyUpdate closed for numeric properties, this time for
 * agent movement: prior to this verb (2026-07-11), every currentLocation
 * change in this dataset's history -- including the two-hop Bonn -> Germany
 * chain already on Kim Meades (urn:event:kim-visit-001/002) -- was written
 * by hand via raw SPARQL UPDATE, with no verb enforcing that a VisitEvent
 * accompanies the change. This verb is that enforcement point.
 *
 * Operates on the flat urn:{dataset}:holons / urn:{dataset}:events graphs,
 * same convention as proposeAgentPropertyUpdate/createAgent -- pass
 * conn.holonsGraph/conn.eventsGraph to override.
 *
 * destinationIri is checked for existence in holonsGraph before anything is
 * written -- a typo'd or not-yet-created destination rejects the whole
 * call rather than leaving an agent pointed at a dangling reference.
 *
 * Chaining: finds the agent's current visit-chain tail -- the VisitEvent
 * with holon:agent = agentIri that has no holon:nextVisit -- and links the
 * new VisitEvent from it. An agent with no prior VisitEvent at all (a
 * first-ever move, or one created before this verb existed) simply starts
 * a new chain with no predecessor; nothing here backfills history that
 * wasn't recorded at the time.
 *
 * Authorisation: self-service when actor.iri === agentIri (an agent moving
 * itself is the common case and should stay frictionless, mirroring
 * joinGroup/leaveGroup's posture) -- Write on datasetAnchor(conn) is
 * required when the actor is moving an agent other than themselves.
 *
 * @param {{sparqlEndpoint: string, gspEndpoint: string, dataset: string, datasetHolonIri?: string, holonsGraph?: string, eventsGraph?: string}} conn
 * @param {string} agentIri
 * @param {{destinationIri: string, actor: {iri: string}, note?: string}} opts
 * @returns {Promise<string>} serialized HolonDataBook
 */
export async function navigateAgent(conn, agentIri, opts) {
  const { destinationIri, actor, note } = opts
  const holonsGraph = conn.holonsGraph ?? `urn:${conn.dataset}:holons`
  const eventsGraph = conn.eventsGraph ?? `urn:${conn.dataset}:events`

  if (actor.iri !== agentIri) {
    await authorise(conn.sparqlEndpoint, datasetAnchor(conn), actor, 'Write')
  }

  // Destination must already exist as a holon -- refuse a dangling move.
  const destQuery = `
    PREFIX holon: <${HOLON_NS}>
    PREFIX rdfs:  <http://www.w3.org/2000/01/rdf-schema#>
    SELECT ?label WHERE {
      GRAPH <${holonsGraph}> {
        <${destinationIri}> a ?type .
        OPTIONAL { <${destinationIri}> rdfs:label ?label }
      }
    } LIMIT 1`
  const { bindings: destBindings } = await runQuery(conn.sparqlEndpoint, destQuery)
  if (destBindings.length === 0) {
    throw new CommandRejected(
      `<${destinationIri}> does not exist in <${holonsGraph}> -- refusing to move an agent there`,
      'NavigateAgentCommand'
    )
  }
  const destLabel = destBindings[0]?.label?.value ?? destinationIri

  // Current location, for the human-readable event label only -- an agent
  // with no prior currentLocation (never yet placed anywhere) is legal.
  const curQuery = `
    PREFIX holon: <${HOLON_NS}>
    SELECT ?loc WHERE { GRAPH <${holonsGraph}> { <${agentIri}> holon:currentLocation ?loc } } LIMIT 1`
  const { bindings: curBindings } = await runQuery(conn.sparqlEndpoint, curQuery)
  const previousLocationIri = curBindings[0]?.loc?.value ?? null

  // Tail of this agent's visit chain, if any -- the VisitEvent with no
  // holon:nextVisit yet. Absent for an agent's first-ever tracked move.
  const tailQuery = `
    PREFIX holon: <${HOLON_NS}>
    SELECT ?event WHERE {
      GRAPH <${eventsGraph}> {
        ?event a holon:VisitEvent ; holon:agent <${agentIri}> .
        FILTER NOT EXISTS { ?event holon:nextVisit ?any }
      }
    } LIMIT 1`
  const { bindings: tailBindings } = await runQuery(conn.sparqlEndpoint, tailQuery)
  const priorEventIri = tailBindings[0]?.event?.value ?? null

  const eventIri = `urn:event:${crypto.randomUUID()}`
  const now = new Date().toISOString()
  const esc = s => s.replace(/"/g, '\\"')
  const agentLocal = agentIri.split(/[/#]/).pop()
  const eventLabel = `${agentLocal} visits ${destLabel}`
...
<${eventIri}> a holon:VisitEvent ;
    rdfs:label "${esc(eventLabel)}"@en ;
    holon:agent <${agentIri}> ;
    holon:visitedHolon <${destinationIri}> ;
    holon:occurredAt "${now}"^^xsd:dateTime${note ? ` ;\n    holon:note "${esc(note)}"` : ''} .
`.trim()
  ...
  if (priorEventIri) {
    await runUpdate(conn.sparqlEndpoint, `
      PREFIX holon: <${HOLON_NS}>
      INSERT DATA { GRAPH <${eventsGraph}> { <${priorEventIri}> holon:nextVisit <${eventIri}> } }
    `.trim())
  }

  await runUpdate(conn.sparqlEndpoint, `
    PREFIX holon: <${HOLON_NS}>
    DELETE { GRAPH <${holonsGraph}> { <${agentIri}> holon:currentLocation ?old } }
    INSERT { GRAPH <${holonsGraph}> { <${agentIri}> holon:currentLocation <${destinationIri}> } }
    WHERE  { OPTIONAL { GRAPH <${holonsGraph}> { <${agentIri}> holon:currentLocation ?old } } }
  `.trim())

  return buildDataBook(
    stampProcess({ id: eventIri, title: `Moved to ${destLabel}`, type: 'holon-visit' }, actor),
    [{
      id: 'visit',
      label: `${previousLocationIri ? previousLocationIri.split(/[/#]/).pop() : '(no prior location)'} -> ${destLabel}`,
      content: eventTurtle
    }]
  )
}
