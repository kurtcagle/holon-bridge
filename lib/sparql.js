...
import { emitWrite } from './write-events.js'
...
function extractJenaMessage(text) {
  if (!text || !text.trim()) {
    // Jena/Fuseki frequently responds with an empty body on transaction
    // conflicts (HTTP 409) and some lock/timeout conditions -- the real
    // reason is only ever in Fuseki's own server-side log in that case.
    // Returning a non-empty placeholder here matters: an empty string is
    // falsy but not nullish, so `err.jenaMessage ?? err.message` upstream
    // does NOT fall through to err.message for '' -- it silently prints
    // nothing after the colon. Never return '' from this function.
    return '(empty response body from Jena -- check the Fuseki server log ' +
           'for the actual error; this is typically a transaction conflict, ' +
           'lock contention, or dataset-unavailable condition)'
  }
  try {
...
// Trim a query/update body down to something readable in a one-line PROCESS label
function briefLabel(text, maxLen = 60) {
...
export async function runQuery(endpoint, query, log = false) {
  ...
  return timedProcess(`Jena SELECT -- ${briefLabel(query)}`, async () => {
...
export async function runConstruct(endpoint, query, log = false) {
  ...
  return timedProcess(`Jena CONSTRUCT -- ${briefLabel(query)}`, async () => {
    const response = await jenaFetch(endpoint, {
      method:  'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
 ...
 *
 * '/sparql' is included alongside '/query' because it's this project's own
 * actual convention (JENA_SPARQL = `${JENA_BASE}/${DATASET}/sparql` in
 * server.js) -- '/query' is the generic Fuseki term the original docstring
 * described, but nothing in this codebase ever produces a URL ending in
 * '/query'. Without this case, every bare runUpdate(conn.sparqlEndpoint, ...)
 * call silently fell through to the broken fallback (appending '/update'
 * onto the *query* path, e.g. '.../data/sparql/update', which doesn't exist)
 * rather than deriving the real sibling endpoint '.../data/update'. This
 * stayed invisible until a code path actually called bare runUpdate() against
 * a live Fuseki instance end-to-end (2026-07-09, proposeAgentPropertyUpdate's
 * property-value write) -- every prior lifecycle verb either used
 * replaceTriples() (which takes its own endpoint) or never reached this
 * specific runUpdate() call in practice (deleteHolon's parent-cleanup path).
 *
 * @param {string} sparqlEndpoint  e.g. http://localhost:3030/ds/sparql
...
export function deriveUpdateEndpoint(sparqlEndpoint) {
  ...
  if (sparqlEndpoint.endsWith('/sparql')) {
    return sparqlEndpoint.slice(0, -'/sparql'.length) + '/update'
  }
...
export async function runUpdate(sparqlEndpoint, update, opts = {}) {
  ...
  return timedProcess(`Jena UPDATE -- ${briefLabel(update)}`, async () => {
    ...
    emitWrite({ graph: null, via: 'update', chars: update.length })
...
export async function pushToGraph(gspEndpoint, graphIri, turtle, mode = 'append') {
  ...
  return timedProcess(`Jena GSP ${method} -- ${graphIri ?? '(default graph)'}`, async () => {
    const response = await jenaFetch(
      url,
      {
        method,
    ...
    emitWrite({ graph: graphIri ?? null, via: 'gsp', chars: turtle.length })
    return result
  })
}
