 ...
 * ---------------------------------------------------
 * UPDATE 2026-07-09: the `data` dataset's holon/event/ontology/shacl
 * graphs (urn:data:holons, urn:data:events, urn:data:ontology,
 * urn:data:shacl) were migrated wholesale from https://ontologist.io/ns/holon#
 * to https://w3id.org/holon/ -- see the migration note in
 * schemas/lifecycle-verbs.schema.json's history. This module's namespace
 * constant (still named HOLON_ONTOLOGIST_NS for import-compatibility --
 * nothing else in this file's public surface changed) now points at
 * https://w3id.org/holon/ to match. If a *different* dataset (ds, chloe,
 * ggsc, etc.) still has holons under the old ontologist.io namespace,
 * fetchHolonProjection() against those will fail role discovery until
 * they're migrated the same way -- this module targets one namespace at
 * a time, not both simultaneously.
 *
 * The flat-graph, holon:isPartOf containment model itself is UNCHANGED --
 * only the base IRI moved. This was a deliberate choice to converge on
 * lifecycle.js's namespace (https://w3id.org/holon/) rather than migrate
 * live Adventure Mode data into lifecycle.js's per-holon schema/scene/
 * events graph triad or its holon:parentHolon containment predicate --
 * see lib/lifecycle.js's own header for the containment-predicate and
 * graph-topology reconciliation this implies on that side.
 *
 * lib/lifecycle.js implements a *different* holon model, now sharing this
 * namespace but not this graph shape: holon:parentHolon (not isPartOf) for
 * containment in its RBAC walk, a three-graph-per-holon layout derived
 * from the holon's own IRI (graphsFor() -> {iri}/schema, {iri}/scene,
 * {iri}/events), RoleBinding ACL, portal-potential lazy realisation, and
 * tombstone deletion. No holon in Fuseki uses the schema/scene/events
 * graph triad or holon:parentHolon -- lifecycle.js's own verbs are not
 * currently wired to any server.js route at all, so nothing calls them
 * against real data yet regardless of namespace.
 *
 * This route is a pragmatic implementation against the model that's
 * actually populated, not a decision that isPartOf/flat-graphs has "won"
 * a reconciliation against lifecycle.js's containment predicate or graph
 * topology -- only the base namespace IRI has been unified so far.
 *
...
export const HOLON_ONTOLOGIST_NS = 'https://w3id.org/holon/'
 ...
 * assuming one.
 *
 * TODO (namespace reconciliation): the base IRI is now shared with
 * lib/lifecycle.js (https://w3id.org/holon/), but the containment
 * predicate (isPartOf vs. parentHolon) and graph topology (flat vs.
 * per-holon schema/scene/events) are still different models. If/when
 * those converge too, this is the place to branch -- e.g. check for a
 * holon:schemaGraph/holon:sceneGraph declaration on the target IRI first,
 * and delegate to something built on lib/lifecycle.js's
 * listHolonContents() when found, falling back to this flat-graph path
 * otherwise.
 *
 * @param {string} sparqlEndpoint
