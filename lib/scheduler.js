 ...
 *
 * sched:taskStatus (added 2026-07-16): per-task suspend/resume, mirroring
 * named rules' hb:ruleStatus (Active/Suspended/Deprecated) rather than
 * inventing a new pattern. Checked in _tick()/_onWrite() before a task is
 * ever considered for firing -- a Suspended task is loaded into memory
 * (visible to list_scheduled_tasks) but never evaluated. Deprecated is the
 * "remove" story, matching deleteHolon's tombstone-by-default convention --
 * no hard-delete verb exists here, mirroring that same discipline. Absent
 * sched:taskStatus (e.g. tasks created before this field existed) defaults
 * to Active, so this is additive -- no existing task's behaviour changes
 * without an explicit status write.
 *
 * sched:summary (added 2026-07-16): an optional short human-readable line
 * on a FiringRecord, populated when an LLMInvocation persona is asked to
 * produce one alongside its Turtle proposal (see _runAction()'s prompt
 * suffix). This is the realistic answer to "can a scheduled task notify
 * someone" -- there is no channel from this process to an open chat
 * session (separate processes, ephemeral SSE connections), so the design
 * is pull, not push: a session asks get_recent_scheduler_activity and
 * reads the summaries of what fired since it last looked, rather than a
 * live notification arriving unprompted. Deliberately reuses
 * urn:scheduler:provenance as the single source of truth rather than a
 * parallel notifications graph that could drift out of sync with it.
 *
 * _resolvePolicyCount() named-graph fix (2026-07-16): found live, during a
 * deliberate scheduler smoke test -- a task with a 20-firing daily-limit
 * policy ran to 26 firings and counting with zero rejections, timestamps
 * cleanly 30s apart throughout (ruling out a race between duplicate
 * scheduler processes as the cause). _resolvePolicyCount()'s SPARQL query
 * had no GRAPH wrapper, so it queried Fuseki's default (unnamed) graph --
 * but every policy this method is ever asked to resolve lives in a named
 * graph (urn:scheduler:tasks for task policies, urn:scheduler:personas for
 * persona policies, matching where each policy is pushed alongside the
 * object that declares it). Every other query in this file that reads
 * scheduler state wraps it in the correct GRAPH clause; this one call site
 * didn't, so it silently saw nothing, returned null, and the `if (taskLimit
 * !== null)` / `if (personaLimit !== null)` guards in _checkRateLimit()
 * skipped the check entirely -- not almost-enforced, not raced, just never
 * evaluated. Fixed by threading the correct graph IRI through from each of
 * _checkRateLimit()'s two call sites rather than guessing or unioning
 * graphs inside _resolvePolicyCount() itself.
 */
...
import { turtleLiteral, turtleLongLiteral, turtleDateTime } from './turtle.js'
...
export class Scheduler {
   ...
   * this is the primitive POST /scheduler/reload (server.js) wraps, so a
   * task pushed via push_turtle or the new create_scheduled_task MCP tool
   * takes effect immediately rather than sitting inert until the next
   * process restart (see scheduler-personas.databook.md's open item on
   * this -- closed 2026-07-16).
   */
  async reload() {
...
SELECT ?t ?actionClass ?triggerType ?intervalMs ?triggerSelect ?persona ?sparql
       ?targetDataset ?targetGraph ?policy ?taskStatus WHERE {
  GRAPH <urn:scheduler:tasks> {
    ...
    for (const b of bindings) {
      ...
      tasks.set(iri, {
        ...
        // Absent -- e.g. a task created before sched:taskStatus existed --
        // defaults to Active, so this field is additive: no task's
        // behaviour changes on upgrade without an explicit status write.
  ...
  async _tick() {
    ...
    for (const task of this._tasks.values()) {
      if (task.taskStatus !== 'Active') continue
  ...
  async _execute(task, triggerContext) {
    ...
    if (task.actionClass === 'ReadOnlyQuery') {
      ...
      await this._recordProvenance(task, persona, triggerContext, { outcome: 'read-only', proposal })
  ...
  async _runAction(task, persona) {
    ...
    if (task.actionClass === 'LLMInvocation') {
      ...
      // Ask for a one-line summary ahead of the Turtle, rather than a
      // separate call -- see the file header's note on sched:summary.
      // "SUMMARY: " prefix keeps parsing trivial and unambiguous even if
      // the model's turtle output happens to start with a comment line.
      const summaryInstruction = '\n\nFormat your entire response as exactly two parts, nothing else: ' +
        'a single line starting with "SUMMARY: " giving a one-sentence plain-English summary of what ' +
        'you are proposing and why, then a blank line, then the Turtle proposal only ' +
        '(no markdown code fences, no other commentary before or after).'
      const response = await anthropicClient().messages.create({
        model,
        max_tokens: 1024,
        system: systemPrompt + summaryInstruction,
        messages: [{ role: 'user', content: task.sparql ?? 'Describe what you would do. Return only Turtle.' }],
      })
      const text = response.content.filter(b => b.type === 'text').map(b => b.text).join('').trim()
      const summaryMatch = text.match(/^SUMMARY:\s*(.+?)\s*\n+/i)
      const summary = summaryMatch ? summaryMatch[1].trim() : null
      const rest = summaryMatch ? text.slice(summaryMatch[0].length) : text
      const turtle = rest.replace(/^```[^\n]*\n?/, '').replace(/\n?```$/, '').trim()
      return { turtle, model, promptHash: _hashString(systemPrompt), summary }
    }
...
SELECT (COUNT(?rec) AS ?c) WHERE {
  GRAPH <urn:scheduler:provenance> {
    ...
    // Graph IRIs match where each policy is actually pushed alongside the
    // object that declares it -- task policies live in urn:scheduler:tasks,
    // persona policies in urn:scheduler:personas. See this file's header
    // for the 2026-07-16 bug this fixes: _resolvePolicyCount() previously
    // queried with no GRAPH wrapper at all, silently seeing nothing.
    const taskLimit = await this._resolvePolicyCount(task.policyIri, 'urn:scheduler:tasks')
    if (taskLimit !== null) {
    ...
    if (persona) {
      const personaLimit = await this._resolvePolicyCount(persona.policy, 'urn:scheduler:personas')
  ...
  /**
   * Resolve an ODRL policy's odrl:count constraint.
   *
   * graphIri is required (2026-07-16 fix) -- every policy this method is
   * asked to resolve lives in a specific named graph (see _checkRateLimit's
   * call sites), never the default graph. Guessing or unioning across
   * graphs here would just trade one silent-failure mode for another; the
   * caller already knows which graph is correct because it knows which
   * kind of policy (task vs persona) it's resolving.
   */
  async _resolvePolicyCount(policyIri, graphIri) {
...
<${recordIri}> a sched:QuarantinedProposal ;
    ...
    sched:reason ${turtleLiteral(reason, { maxLength: 1000 })} ;
    sched:quarantinedAt ${turtleDateTime(new Date())} ;
    sched:proposedTurtle ${turtleLongLiteral(proposal.turtle ?? '')} .
`.trim()
    // Best-effort, matching _recordProvenance's own discipline below. Before
    // 2026-07-25 this call was unguarded while the provenance write was wrapped,
    // so an identical failure -- the same malformed literal, since both records
    // embed the same reason string -- was fatal here and merely logged there.
    // A bookkeeping write that cannot be serialised must not take down the tick
    // that produced it; the quarantine DECISION still stands and is still
    // recorded in provenance, which is the record that matters for audit.
    try {
      await pushToGraph(this.gspEndpoint, 'urn:scheduler:quarantine', turtle, 'append')
    } catch (err) {
      console.warn(`[scheduler] quarantine write failed for ${task.iri}: ${err.message}`)
    }
...
<${recordIri}> a sched:FiringRecord ;
    ...
    sched:triggerType ${turtleLiteral(triggerContext.triggerType)} ;
    sched:outcome ${turtleLiteral(outcome)} ;
    ${reason ? `sched:reason ${turtleLiteral(reason, { maxLength: 1000 })} ;` : ''}
    ${proposal?.model ? `sched:model ${turtleLiteral(proposal.model)} ;` : ''}
    ${proposal?.promptHash ? `sched:promptHash ${turtleLiteral(proposal.promptHash)} ;` : ''}
    ${proposal?.summary ? `sched:summary ${turtleLiteral(proposal.summary, { maxLength: 500 })} ;` : ''}
    sched:firedAt ${turtleDateTime(now)} .
`.trim()
